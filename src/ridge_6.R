...
for (l in lambdas)
{
  ...
  {
    # Which features are the most correlated to the output? Keep the top 10 (not counting sale price itself)
    correlations <- names(x_train)[order(cor(x_train, y_train), decreasing = T)[2:(c+1)]]
    ...
    for (i in 1:num_folds) {
      ...
      model<- linearRidge(as.formula(paste("SalePrice ~. + (", paste(correlations, collapse="+"), ")^2")), data = X_tr, scaling = "none", lambda = l)
    ...
    print(paste("CORS=", c, "; LAMBDA=", l, "; CV error=", round(mean(cv_errors), digits=4), " ; std dev=", round(sd(cv_errors), digits=4)))
  }
}
